...
function get_category_news($database_connection, $page_size = 5, $page_current = 1){
    //构造查询所有新闻的SQL语句
    $count_all_sql = 
...
function get_latest($database_connection, $news_id = 0, $page_size = 5, $page_current = 1){
    //变量声明
    $start = ($page_current-1)*$page_size; 
...
function get_main_news($database_connection, $news_id = 0, $page_size = 5, $page_current = 1){
    //变量声明
    $start = ($page_current-1)*$page_size; 
...
function get_matching($database_connection, $keyword = "", $page_size = 5, $page_current = 1, $category_id = ""){
...
function get_most_viewed($database_connection, $news_id = 0, $page_size = 5, $page_current = 1){
    //变量声明
    $start = ($page_current-1)*$page_size; 
...
function get_news_count($database_connection, $keyword = "", $category_id = ""){
    $keyword_search = addslashes($keyword);
...
function get_popular($database_connection, $news_id = 0, $page_size = 5, $page_current = 1){
    //变量声明
    $start = ($page_current-1)*$page_size; 
...
function get_related($database_connection, $news_id = 0, $page_size = 5, $page_current = 1){
    //变量声明
    $start = ($page_current-1)*$page_size; 
...
function get_tab_content_news($database_connection, $news_id = 0, $page_size = 5, $page_current = 1){
    //变量声明
    $start = ($page_current-1)*$page_size; 
...
function get_top_news($database_connection){
    //变量声明
    $page_size = 4;
    $page_current = 3;
    $start = ($page_current-1)*$page_size; 
    ...
    (SELECT news_id, title, thumbnail FROM (
        SELECT news.news_id, news.category_id, news.title, news.thumbnail, SUM( news.clicked ) AS total_views, SUM( news.clicked ) AS total_clicked, COUNT( review.review_id ) AS total_comments, HOUR( TIMEDIFF( NOW( ) , news.publish_time 		   ) ) AS hour_since_published, 1 - ( HOUR( TIMEDIFF( NOW( ) , news.publish_time ) ) / ( 24 *7 ) ) AS freshness_score,  SUM( news.clicked ) * 0.4 + COUNT( review.review_id ) * 0.2 + ( 1 - ( HOUR( TIMEDIFF( NOW( ) , news.publish_time ) ) / ( 24 *7 ) ) ) * 0.4
    AS trendingscore FROM ( news LEFT JOIN review ON news.news_id = review.news_id ) WHERE title NOT LIKE '%习近平%' GROUP BY news.news_id, news.category_id, hour_since_published , freshness_score ORDER BY trendingscore DESC LIMIT 1
    ) as t2
    )
    UNION ALL
    (SELECT news_id, title, thumbnail FROM (
        SELECT * FROM news WHERE title NOT LIKE '%习近平%' ORDER BY publish_time DESC LIMIT 1,1
    ) as t3
    )
    ";
...
function get_xi($database_connection, $page_size = 5, $page_current = 1, $category_id = ""){
    $start = ($page_current-1)*$page_size; 
